...
class Experiment:
    ...
    def _read_hyperparameters_from_config_pd(self, method):
        ...
        # todo: read hyperparameters from self.methodconfig; depends on both method and dataset
        _params = {}
        for param in self.methodconfig['hyperparameters_pd'][method]:
            _params[param] = self.methodconfig['hyperparameters_pd'][method][param][0]
        print(f"*Selected hyperparameters ({method})* {_params}")
        return _params
