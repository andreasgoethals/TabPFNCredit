#!/usr/bin/env python3
"""
GPU Orchestrator for Experiment 2: Learning Curve Analysis

This script:
1. Reads enabled GPU methods and datasets from config
2. Builds list of all GPU tasks (method × dataset combinations)
...
def build_gpu_task_list(config):
    """
    Build list of all GPU tasks for learning curve analysis.
...
def main():
    parser = argparse.ArgumentParser(
        description='Run GPU methods for Learning Curve Analysis (Experiment 2)'
    )
    ...
    print(f"\n{'='*70}")
    print(f"EXPERIMENT 2: GPU ORCHESTRATOR - LEARNING CURVE ANALYSIS")
    print(f"{'='*70}")
